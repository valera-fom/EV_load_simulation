...
try:
    from pages.components.dataset_manager import render_dataset_selection_ui
except ImportError as e:
    st.error(f"Failed to import DatasetManager: {e}")
    # Create a dummy function if import fails
    def render_dataset_selection_ui():
        st.error("Dataset Manager not available")
        return None
...
with st.sidebar:
 ...
 # Reverse Simulation (Capacity Analysis)
    ...
    # Dataset Selection - Now handled by separate module
    power_values, data_source, grid_mode = render_dataset_selection_ui()
